...
export const signUp = async(req,res)=>{
    ...
    try{
        ...
        if(existingUser){
            return res.status(200).json({success: true, message: "User already registered", existingUser});
        }

        const newUser = new User(user);
        await newUser.save();

        res.status(201).json({success: true, message: "User successfully registered!", newUser});
    }catch(error){
